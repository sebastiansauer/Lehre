...
# `y ~ x_c` - Centering a metric independent variable
...
Advise: Center all numeric *input* variables in your model.
...
```{r}
check_model(lm_weight_flipper_c)   # from easystats
```


# Non-normal data - simulation study



Let's simulate some non-normal dat to check it' influence on the modelling.
...
x <- 1:n   # 1,2,3,...
sigma <- 5
e <- sigma * runif(n = n)  # random uniformly distributed
e2 <- sigma * rnorm(n = n)  # normally distributed
y <- x + e
y2 <- x + e2
```
...
Run the model with `e`, *non* normal errors:

```{r}
lm_nonnormal <-
  lm(y ~ x, data = non_normal_df)

parameters(lm_nonnormal)
```


```{r}
estimate_relation(lm_nonnormal) |> 
  plot() +
  geom_point(aes(x = x, y = y), data = non_normal_df)
```


Run the model with e2, *normal* error:


```{r}
lm_normal <-
  lm(y2 ~ x, data = non_normal_df)

parameters(lm_normal)
```


```{r}
estimate_relation(lm_normal) |> 
  plot() +
  geom_point(aes(x = x, y = y2), data = non_normal_df)
```

# `y ~ x1_z + x2_z` - multiple metric input variables
