...
- Der *Tolerenzbereich* einer Lösung liegt bei 5% des Wertes der Musterlösung *oder* bei `0.1*sd(y)`, je nachdem welcher der beiden Werte kleiner ist (y: vorherzusagende Variable). Tolerenzbereiche werden verwendet für Aufgaben mit Zufallszahlen: Lösungen solcher Aufgaben können schwanken. Wenn Sie allerdings den angegebenen Seed-Wert gesetzt haben, so sollte Ihre (korrekte) Lösung exakt der Musterlösung entsprechen.
...
Nutzen Sie die in der Prüfung zur Verfügung gestellten Funktionen/Buttons dafür.
